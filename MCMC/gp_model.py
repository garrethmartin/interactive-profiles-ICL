...
class MultiOutputGPModel:
    ...
    def run_MCMC(self, X_std, Y_std, rank_per_kernel=None, random_seed=None,
        ...
        with pm.Model() as model:
            ...
            for k, kernel_i in enumerate(self.kernel_constructors):
                cov_input = (amp[k] ** 2) * kernel_i(input_dim=full_input_dim, ls=ls[k], active_dims=[0, 1])
                if n_kernels == 1:
                ...
                coregion_k = pm.gp.cov.Coregion(input_dim=full_input_dim, W=W_k, kappa=kappa_k, active_dims=[2])
                cov_sum += cov_input * coregion_k

            sigma = pm.HalfNormal("sigma", sigma=0.075)
            mogp = pm.gp.Marginal(cov_func=cov_sum)
            mogp.marginal_likelihood("y", X=X_aug, y=Y_aug, sigma=sigma)

            # ADVI + jittered starts
            advi_iters = 40_000
            approx = pm.fit(advi_iters, method="advi", obj_optimizer=pm.adam(learning_rate=1e-3), random_seed=random_seed)
    ...
    def evaluate_posterior_predictive(self, model, mogp, idata, X_aug, Y, scaler_Y, D, N, random_seed=None):
        ...
        for d in range(D):
            ...
            axes[d, 0].plot([Y[:, d].min(), Y[:, d].max()], [Y[:, d].min(), Y[:, d].max()], ls='--', color='grey')
        ...
        fig.savefig('tests.pdf', bbox_inches='tight')
    ...
    def gp_grid_realizations_multi_kernel(self, idata, scaler_X, scaler_Y, X_std, Y_std,
                                          mstar_range, nu_range,
                                          n_m=50, n_nu=50,
                                          n_posterior_draws=300,
                                          kernel_constructors=None,
                                          jitter=1e-8,
                                          random_seed=None, n_jobs=10):
        """Generate grid realizations from the multi-kernel GP posterior."""
        ...
        grid_m, grid_nu = np.meshgrid(m_grid, nu_grid, indexing='ij')
        X_grid_phys = np.column_stack([grid_m.ravel(), grid_nu.ravel()])
        ...
        for c, d in tqdm(zip(chains, draws), total=len(chains)):
            ...
            for k in range(n_kernels):
                ...
                def chunk_kernel(X_chunk):
                    return kernel_fn(X_chunk, X_train_phys, ls_vec)

                X_chunks = np.array_split(X_grid_phys_std, max(1, n_jobs))
                K_sx_chunks = Parallel(n_jobs=n_jobs, prefer="threads")(delayed(chunk_kernel)(chunk) for chunk in X_chunks)
            ...
            pred_real = sample.reshape(D, -1).T
            pred_all.append(pred_real)

        pred_all = np.array(pred_all)
        pred_real_orig = np.array([scaler_Y.inverse_transform(pred_all[i]) for i in range(pred_all.shape[0])])
        return m_grid, nu_grid, pred_real_orig

def fit_multioutput_gp(
    X, Y, random_seed=1, save_hdf5=True, fname='gp_predictions.h5',
    n_jobs=10, n_chains=10, n_tune=10_000, n_draws=2_500, overwrite=False,
        clip_mins=None, clip_maxs=None, rank_per_kernel=1
):
    """
    Fits a multi-output Gaussian Process (GP) model to the input data, performs MCMC sampling,
    evaluates posterior predictive checks, and generates grid-based realizations of the GP.

    Parameters
    ----------
    clip_mins : list or array, optional
        Minimum values for each output dimension (use None for no min).
    clip_maxs : list or array, optional
        Maximum values for each output dimension (use None for no max).
    """
    if os.path.exists(fname) and not overwrite:
        with h5py.File(fname, 'r') as f:
            m_grid_2 = f['m_grid'][()]
            nu_grid_2 = f['nu_grid'][()]
            pred_real_2 = f['pred_realizations'][()]
        return {
            'model': None,
            'mogp': None,
            'idata': None,
    ...
    labels = ['log10(e_DM)', 'log10(h_DM)', 'log10(f_strip_DM)', 'log10(e_star)', 'log10(h_star)', 'log10(f_strip_star)']
    for dim_idx, (label, ax) in enumerate(zip(labels, axs.flatten())):
        sc = ax.scatter(X_std[:, 0], X_std[:, 1], c=Y_std[:, dim_idx], s=400, edgecolors='k')
        ax.set_title(label)
        plt.colorbar(sc, ax=ax)

    gp = MultiOutputGPModel(random_seed=random_seed)
    model, mogp, idata, X_aug, Y_aug = gp.run_MCMC(X_std, Y_std,
                                                   n_jobs=n_jobs, n_chains=n_chains,
                                                   n_tune=n_tune, n_draws=n_draws,
                                                   rank_per_kernel=rank_per_kernel)
    tests = gp.evaluate_posterior_predictive(model, mogp, idata, X_aug, Y, scaler_Y, D, N)

    m_range = [X[:, 0].min() * 0.98, X[:, 0].max() * 1.0]
    nu_range = [0.0, 1.0]
    m_grid, nu_grid, pred_real = gp.gp_grid_realizations_multi_kernel(
        idata, scaler_X, scaler_Y, X_std, Y_std,
        mstar_range=m_range,
        nu_range=nu_range,
        n_m=100, n_nu=100, n_posterior_draws=n_draws*n_chains, random_seed=random_seed,
        kernel_constructors=gp.kernel_constructors, n_jobs=n_jobs)
    
    # Flexible per-output clipping
    if clip_mins is not None or clip_maxs is not None:
        for d in range(pred_real.shape[2]):
            min_val = clip_mins[d] if clip_mins is not None else None
            max_val = clip_maxs[d] if clip_maxs is not None else None
            pred_real[:, :, d] = np.clip(pred_real[:, :, d], a_min=min_val, a_max=max_val)
